#!/usr/bin/python3
"""Backtest 3 pares — GBPJPY ICT, GBPUSD LC, EURUSD LC — 30 dias M15."""
import yfinance as yf
import json
from datetime import datetime, timedelta
from pathlib import Path

OUTPUT = Path.home() / '.hermes' / 'forex' / 'backtest_3pares.json'
                            ...
                            if gap >= MIN_FVG_PIPS:
                                ...
                                if candle_hour in allowed_hours:
                                    all_signals.append({
                                        ...
                                        'fvg_pips': gap, 'idx': i, 'hour': candle_hour
                                    })
                                break
                    break
...
def backtest(use_crt=False, use_killzone=True):
    end = datetime.now()
...
if __name__ == '__main__':
    print("=" * 65)
    print("BACKTEST — 3 Pares, 30 dias, M15, gap>=3p, killzones ativos")
    print("=" * 65)

    r1, t1 = backtest(use_crt=False, use_killzone=True)
    r2, t2 = backtest(use_crt=True, use_killzone=True)
    ...
    diff_t = t2['trades'] - t1['trades']
    diff_wr = t2['wr'] - t1['wr']
    diff_pnl = t2['pnl'] - t1['pnl']
    print(f"CRT impact:          {diff_t:+3d}T | WR {diff_wr:+.1f}pp | PnL {diff_pnl:+}p")
    print(f"{'='*65}")
    ...
    OUTPUT.write_text(json.dumps({
        ...
        'gap': MIN_FVG_PIPS, 'killzones': True
