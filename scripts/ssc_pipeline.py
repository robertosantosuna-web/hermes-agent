...
Fase 1: Baixa TODOS os vídeos do canal SSC
Fase 2: Transcreve via faster-whisper (CPU)
Fase 3: Extrai conceitos e regras aplicáveis ao bot
"""
...
SUMMARY_PATH = HOME / '.hermes' / 'ssc_summary.json'
...
def extract_rules(transcripts):
    ...
    concept_patterns = {
        ...
        "FVG": r'(?i)(FVG|Fair Value Gap|gap de valor|gap\s+de\s+valor)',
    ...
    for concept, pattern in concept_patterns.items():
        ...
        if matches:
            # Extrair snippets ao redor
            snippets = []
            for m in matches[:5]:  # Top 5 exemplos
                idx = full_text.find(m)
                if idx >= 0:
                    snippet = full_text[max(0,idx-80):idx+len(m)+200]
                    snippets.append(snippet.strip()[:300])
            
            rules["concepts"][concept] = {
                "count": len(matches),
                "examples": snippets
            }
    
    # Extrair regras numéricas
...
if __name__ == '__main__':
    ...
    p.add_argument('--download', action='store_true', help='Baixar vídeos')
    p.add_argument('--transcribe', action='store_true', help='Transcrever')
    p.add_argument('--extract', action='store_true', help='Extrair regras')
    p.add_argument('--all', action='store_true', help='Pipeline completo')
    args = p.parse_args()
    
    if args.all or (not args.download and not args.transcribe and not args.extract):
    ...
    if args.extract:
        if not transcripts:
            # Carregar transcrições existentes
            transcripts = []
        ...
        print(f"Vídeos: {rules['videos_processed']}")
        print(f"Conceitos extraídos: {len(rules['concepts'])}")
        print(f"Total mencões: {sum(c['count'] for c in rules['concepts'].values())}")
